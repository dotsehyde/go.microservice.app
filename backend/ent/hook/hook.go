...
// The NationalityFunc type is an adapter to allow the use of ordinary
// function as Nationality mutator.
type NationalityFunc func(context.Context, *ent.NationalityMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f NationalityFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.NationalityMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.NationalityMutation", m)
}
